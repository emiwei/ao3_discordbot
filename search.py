...
class Search:
    ...
    def update(self):
        ...
        if self.pages > 1:
            randompage = random.randint(1, self.pages)
            new_url = url.replace("page=1", "page=" + str(randompage))
            if self.session is None:
                req = requester.request("get", new_url)
            else:
                req = self.session.get(new_url)
        ...
        for work in results.find_all("li", {"role": "article"}):
            ...
            new, workid = get_work_from_banner(work)
            new._session = self.session
            works.append((new, workid))
